...
#' Returns a list of t-scores (@t-value), p-scores (@p-value), and BF (either @BF10, @BFplus0, or @BFmin0)
#'
...
#'
#' tbl$bf <- bfttest(tbl$sums, alternative = "two.sided", mu = 50, nstart = 10, prior.loc = 0.1, prior.r = 0.05)$BF10
#' @export

bfttest <- function(data, alternative = c("two.sided", "less", "greater"), mu = 0, paired = FALSE, nstart = 3, prior.loc = 0, prior.r = 0.1){
  ...
  if (alternative=="less"){
    bft.out <- list("t-value" = tlist, "p-value" = plist, "BFmin0" = BFmin0)
  } else if (alternative=="greater"){
    bft.out <- list("t-value" = tlist, "p-value" = plist, "BFplus0" = BFplus0)
  } else {
    bft.out <- list("t-value" = tlist, "p-value" = plist, "BF10" = BF10)
  }
  return(bft.out)
}
